  ...
  <div class="p-4 size-full">
    ...
    >
      ...
      <template #override-node="slotProps">
        <ViewerNode
          v-bind="{ ...slotProps, ...network.extra[slotProps.nodeId] }"
        />
      </template>
    </v-network-graph>
  </div>
...
async function analyze(gStore: SolidStore) {
  const subjects = arrayUnique(
    gStore.getQuads().map((quad) => quad.subject.value),
  ).filter(
    (subject) =>
      !gStore.statement(subject, "rdf:type", "crdt:Metadata") &&
      !gStore.statement(subject, "rdf:type", "crdt:SetPropertyOperation"),
  );
  ...
  console.log(
    await Promise.all(
      subjects.map(async (subject) => [
        subject,
        gStore.statement(subject, "rdf:type")?.object.value,
        await Promise.all(
          arrayUnique(
            gStore
              .statements(undefined, "crdt:resource", subject)
              .map((statement) => statement.subject.value),
          ).map(async (subject) => {
            const jsonld = await quadsToJsonLD(gStore.statements(subject));

            return formatJsonLD(jsonld, {
              resourceId: subject,
              context: {
                "@vocab": "https://vocab.noeldemartin.com/crdt/",
              },
            });
          }),
        ),
...
watch(
  () => graph,
  async () => {
    const jsonld = await quadsToJsonLD(graph);
    const gStore = new SolidStore(graph);

    await analyze(gStore);

    // console.log(jsonld);

    // console.log(
    //   await formatJsonLD(jsonld, {
    //     resourceId: "https://example.com/#it",
    //   }),
    // );
  },
  { immediate: true },
...
const network = computed(() => {
  const subjects = Array.from(
    new Set(store.getQuads().map((quad) => quad.subject.value)),
  ).map((id) => new RDFNamedNode(id));
